...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class', 
        title='Total Success Launches %s'%(entered_site))
    return fig
...
def get_scatter_chart(entered_site,minmax_payload):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_site]#.groupby(['Launch Site','class']).size().reset_index(name='class count')
        filtered_df = filtered_df[(filtered_df['Payload Mass (kg)']>=minmax_payload[0]) & (filtered_df['Payload Mass (kg)']<=minmax_payload[1]) ]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', 
        y='class', 
        color="Booster Version Category", 
